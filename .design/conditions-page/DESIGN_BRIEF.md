...
| `WeekGrid`                            | **New**       | Days across, products down. Tolerates ragged row lengths; a product with no forecast is absent, not blank. **Waves are not such a product** — only NDBC is observation-only, and CDIP MOP publishes an hourly wave forecast ~10 days out at ~100 m alongshore, so waves get a reserved row |
| `TideWeek`                            | **New**       | The first live row: a week of lowest lows                                                                                                                                                                                                                                                  |
| `DaylightWeek`                        | **New**       | The second live row: sunrise and sunset for each of the seven days, computed in-repo from the beach's own coordinates. No API, per `docs/plans/conditions-tool.md`. Added mid-flight; see the addendum in `TASKS.md`                                                                       |
