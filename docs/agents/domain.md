...
This is a **single-context** repo: one `CONTEXT.md` and one `docs/adr/`, both
at the repo root.
...
If `CONTEXT.md` doesn't exist yet, **proceed silently**. Don't flag its
absence; don't suggest creating it upfront. The producer skill
(`/grill-with-docs`) creates it lazily when terms or decisions actually get
resolved.
...
> _Contradicts ADR-0002 (gate runner in Node) — but worth reopening because…_
